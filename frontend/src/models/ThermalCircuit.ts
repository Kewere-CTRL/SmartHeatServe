...
export const transformThermalCircuit = (thermalCircuits: ThermalCircuit[]): ObjectLink[] => {
    console.log(thermalCircuits)
    return thermalCircuits.map((thermalCircuit) => ({
        label: thermalCircuit.label,
        hiddenField: 'Тепловой контур',
        link: `/building/thermalCircuit/${thermalCircuit.id}`,
    }));
};
