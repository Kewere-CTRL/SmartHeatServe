...
const ModalTemplate: React.FC<ModalTemplateProps> = ({
    ...
    const handleWheel = (e: React.WheelEvent) => {
        if (modalContentRef.current && useArtificialScroll) {
            e.preventDefault();
            const delta = e.deltaY;
            const scrollSpeedFactor = 3;
            modalContentRef.current.scrollTop += delta / scrollSpeedFactor;
        }
    };
            ...
            >
                <div className="bg-gray-800 px-6 py-4 rounded-t-lg w-full">
                    <div className="flex justify-between items-center">
                        ...
                        <button
                            onClick={handleClose}
                            className="text-gray-500 hover:text-gray-400 transform transition duration-300 ease-in-out hover:animate-rotate180"
                        >
                            <FaTimes size={20}/>
                        </button>
                    </div>
                ...
                <div
                    ref={modalContentRef}
                    className="p-6 space-y-2 max-h-[60vh] overflow-y-auto"
                    onWheel={handleWheel}
                >
                    {children}
                </div>
